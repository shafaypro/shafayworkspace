...
def file_read():
	...
	if location_file is "":
		...
		'''
		location_file = "https://vincentarelbundock.github.io/Rdatasets/csv/texmex/rain.csv"  # WebFile def
...
def database_writer(data):
	...
	connection = sqlite3.connect(
		'Rain.sqlite')  # The connection is established from the python file to the data
	cursor_connector = connection.cursor()  # this activates the cursor used to run queries
	cursor_connector.execute(
		'''CREATE TABLE IF NOT EXISTS Rain (id INTEGER UNIQUE, rain REAL) ''')
	cursor_connector.execute('''INSERT INTO Rain(id,rain) values (?,?)''',
							 (data[0], data[1]))
...
def Day():
		...
		'Rain.sqlite')  # The connection is established from the python file to the data
		cursor_connector = connection.cursor()  # this activates the cursor used to run queries
		cursor_connector.execute('''Select id from Rain''')
...
def Rain_meter():
		...
		'Rain.sqlite')  # The connection is established from the python file to the data
		cursor_connector = connection.cursor()  # this activates the cursor used to run queries
		cursor_connector.execute('''Select rain from Rain''')
...
def main():
	...
	result_upcoming = passengercheck_between(1949, 1959)
	xaxisdata = Day()  # recieves the year of the passnegers
	yaxiddata = Rain_meter()  # recieves the passengers numbers
	scatterplot_creator(xaxisdata, yaxiddata, 'Day', 'Rain', 'Rain STATS')
	plot_creator(xaxisdata, yaxiddata, 'Day', 'Rain', 'Rain STATS')
